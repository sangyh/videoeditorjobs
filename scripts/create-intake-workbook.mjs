...
const communityPostSeedRows = [
  ...
  [
    ...
    "https://videoeditorjobs.com/blog/how-to-price-video-editing-work/?utm_source=facebook&utm_medium=group&utm_campaign=blog_guide",
    "facebook",
    "group",
    "blog_guide",
    "",
...
function addSetupSheet(workbook) {
  ...
  const rows = [
    ...
    [
      "Apps Script",
      "Paste docs/google-sheets-apps-script.js into Extensions > Apps Script, save, run setup(), run seedCommunityPosts(), approve Sheets/send-email permissions, then run launchHealthCheck(). Confirm ok: true and scriptVersion: vej-2026-06-29-50p-french-editor before deploying.",
    ],
    [
      "Test Submission",
    ...
    [
      "Deploy",
      "Deploy Apps Script as a Web app. Execute as Me. Access Anyone. Copy the /exec URL, open it, and confirm ok: true plus scriptVersion: vej-2026-06-29-50p-french-editor.",
    ],
    ["Environment", "Run npm run configure:endpoint -- <exec-url>, then deploy with VEJ_INTAKE_ENDPOINT set in production."],
    [
      "Smoke Test",
      "Run npm run smoke:intake after /exec deploy. Confirm health.scriptVersion is vej-2026-06-29-50p-french-editor and one editor row plus one hiring row land with triage and confirmation results.",
    ],
    [
      "Launch Gate",
      "Run npm run launch:ready -- --require-endpoint, then npm run smoke:live -- https://videoeditorjobs.com --require-endpoint after deploy. Current local bundle verifies 50 crawlable pages.",
    ],
