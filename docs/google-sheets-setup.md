...
After deployment, open the `/exec` URL in a browser. The `doGet()` health check should return JSON with `ok: true`, `scriptVersion: "vej-2026-07-20-intake-summary"`, the configured spreadsheet ID, and the expected sheet names. Confirm that version matches `contract.scriptVersion` from `npm run prepare:apps-script`.
...
- JSON output includes `health.scriptVersion: "vej-2026-07-20-intake-summary"`.
