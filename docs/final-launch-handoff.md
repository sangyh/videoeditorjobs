...
8. Run `launchHealthCheck()` and confirm `ok: true` and `scriptVersion: "vej-2026-07-20-intake-summary"`.
...
Confirm `contract.scriptVersion` from `npm run prepare:apps-script`, `launchHealthCheck().scriptVersion`, and `npm run smoke:intake` `health.scriptVersion` all match `vej-2026-07-20-intake-summary`.
